...
import {
  ...
  getCognitoIdentityUUID
} from "../../lib/aws/aws-cognito-federated-identity";
...
import {
  createParameter,
  updateParameter
} from "../../lib/aws/aws-parameter-store/put-parameter";
import {
  createAwsServiceClients,
  ServiceName
} from "../../lib/aws/aws-service-clients";
...
export class LoginItemParameterStoreRepository implements LoginItemRepository {
  ...
  public static async build(
    idToken: string
  ): Promise<LoginItemParameterStoreRepository> {
  ...
  public async update(
    ...
    if (
      JSON.stringify(updatedItem.getDecryptedData()) ===
      JSON.stringify(oldItem.getDecryptedData())
    )
      return oldItem;
