...
const stringPattern = new RegExp(/^[a-zA-Z0-9!\"#$%&'()*+,-./:;<=>?@[\\\]\^_`{\|}~]{1,50}$/m)
        ...
        case 'PATCH': {
          return Joi.object({
            data: {
              ...
              id: Joi.valid(req.query.id).messages({"any.only": '"data.id" must be equal to id in URL'}),
              attributes: loginItemAttributesSchema
            }
          }).options({ presence: 'required' })
        }
 ...
 */
export async function validateMiddleware(req: NextApiRequest, res: NextApiResponse): Promise<void> {
