...
import {
  ...
  getCognitoIdentityRegion,
  getCognitoIdentityUUID
} from "../../lib/aws/aws-cognito-federated-identity";
import { logger } from "../../lib/logger/logger";
import {
  LoginItem,
  LoginItemDecryptedData
} from "../../models/login/login-item";
...
const setupMocks = (
  mockOverrides?: any
): Array<LoginItemParameterStoreRepository> => {
  // @ts-ignore
...
describe("login item ParameterStore repository", () => {
  ...
  it("should create item", async () => {
    ...
    expect(response.lastModifiedDate.toTimeString()).toBe(
  ...
  it("should throw NotFoundException on get non-existent item", async () => {
    ...
    await expect(() => repo.get("nonExistentId")).rejects.toThrow(
      NotFoundException
    );
  });
  ...
  it("should throw NotFoundException on update non-existent item", async () => {
    ...
    await expect(() =>
      repo.update("nonExistentId", "newTitle")
